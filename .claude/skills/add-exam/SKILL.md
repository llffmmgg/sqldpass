...
2. 총 문항 수
3. 과목 구성 (과목명 + 과목별 문항 수)
4. 시험 시간
5. 합격 기준 (평균 점수 + 과목별 최저 점수)
6. 시험 형태 (4지선다 / 단답형+서술형 / 혼합)
7. 과목별 세부 토픽 목록
8. **토픽별 출제 문항 수** — 가중 분포의 핵심

**선택 필드:**
9. 2024년 이후 개편 사항
10. 기존 시험과 겹치는 과목 여부

수집 방식은 **A/B/C 중 사용자가 선택**한다. 기본 권장은 **B**(토픽 분포는 사용자가 즉답하기 어려운 경우가 많음):

**옵션 A — 사용자에게 직접 질문**
- 필요 정보를 순서대로 질문, 사용자가 답변
- 사용자가 해당 시험을 잘 알고 분포까지 명확하면 추천

**옵션 B — LLM 프롬프트 생성 (기본 권장)**
- 아래 스키마로 스펙을 뽑는 프롬프트를 작성해 사용자에게 전달
- 사용자가 웹 Claude/ChatGPT에서 실행 → 결과 JSON을 붙여넣으면 Step 2로
  ...
  ```
  {
    ...
    "questionFormat": {
      "type": "MCQ_ONLY|SHORT_ANSWER_ONLY|MIXED",
      "mcqCount": int,
      "shortAnswerCount": int,
      "descriptiveCount": int,
      "mcqOptions": int
    },
    "recentChanges": string,
    "overlapsWithOtherCerts": string,
    "uncertain": bool,
    "sources": [string]
  }
  ```
  ...
  프롬프트 제약 (반드시 포함):
  1. subjects[].questionCount 합 = totalQuestions
  2. topics[].questionCount 합 = 해당 subject.questionCount
  3. 토픽은 과목당 6~12개
  4. 2024년 이후 개편 반영, 불확실 항목은 `uncertain: true` + sources 기재
  5. JSON 외 텍스트 금지

**옵션 C — 내장 지식 기반 즉시 생성**
- 내가 해당 시험을 충분히 알면 위 JSON 스키마대로 직접 채움
- 정확도 보장 못 하므로 사용자 검수 필수
