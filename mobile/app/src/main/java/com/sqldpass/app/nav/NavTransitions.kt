 ...
 *
 * - 5탭 사이: fade + 미세 scale (위계 동등, 빠른 전환 + 약한 깊이감)
 * - 풀스크린(Runner / PassPlus) 진입: 슬라이드 인 오른쪽→왼쪽 + 페이드 (iOS push 톤)
 * - 풀스크린 뒤로가기: 슬라이드 아웃 오른쪽으로 (대칭)
 *
 * Material 표준 280ms / 페이드 220ms.
 */

private const val FADE_DUR = 160
private const val TAB_SCALE_DUR = 200
private const val SLIDE_DUR = 280
private const val SLIDE_FADE_DUR = 220

fun tabFadeEnter(): EnterTransition =
    fadeIn(animationSpec = tween(FADE_DUR)) +
        scaleIn(animationSpec = tween(TAB_SCALE_DUR), initialScale = 0.98f)

fun tabFadeExit(): ExitTransition =
    fadeOut(animationSpec = tween(FADE_DUR)) +
        scaleOut(animationSpec = tween(TAB_SCALE_DUR), targetScale = 1.02f)
