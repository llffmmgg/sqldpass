...
Usage:
    python3 scripts/execute.py <phase-dir> [--agent claude|codex] [--push]
"""
...
class StepExecutor:
    ...
    def __init__(self, phase_dir_name: str, *, agent: str = "claude", auto_push: bool = False):
        self._root = str(ROOT)
        ...
        if self._agent not in {"claude", "codex"}:
            print(f"ERROR: unsupported agent '{self._agent}'")
            sys.exit(1)
    ...
    def _load_guardrails(self) -> str:
        ...
        if agents_md.exists():
            sections.append(f"## 저장소 규칙 (AGENTS.md)\n\n{agents_md.read_text(encoding='utf-8')}")
...
def main():
    ...
    parser.add_argument("--agent", choices=["claude", "codex"], default="claude", help="Agent CLI to run each step")
    parser.add_argument("--push", action="store_true", help="Push branch after completion")
    args = parser.parse_args()

    StepExecutor(args.phase_dir, agent=args.agent, auto_push=args.push).run()
